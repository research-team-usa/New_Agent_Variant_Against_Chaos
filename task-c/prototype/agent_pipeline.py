...
def log_trace(run_id: str, agent_id: str, event_type: str, status: str, payload: Dict[str, Any]):
...
class ReadTask(AgentTask):
    def execute(self, context: PipelineContext) -> TaskResult:
        log_trace(context.run_id, self.task_id, "action_start", "success", {"action": "parse_text"})
        output = f"Parsed: {context.input_data}"
        log_trace(context.run_id, self.task_id, "action_end", "success", {"output_length": len(output)})
        return TaskResult(status="success", output_data=output)

class RewriteTask(AgentTask):
    def execute(self, context: PipelineContext) -> TaskResult:
        log_trace(context.run_id, self.task_id, "action_start", "success", {"action": "structure_text"})
        input_from_read = context.upstream_results.get("ID-001-READ", "")
        output = f"<structured>{input_from_read}</structured>"
        log_trace(context.run_id, self.task_id, "action_end", "success", {"status": "formatted"})
        return TaskResult(status="success", output_data=output)
...
class ConvertTask(AgentTask):
    ...
    def execute(self, context: PipelineContext) -> TaskResult:
        log_trace(context.run_id, self.task_id, "action_start", "success", {"action": "render_pdf"})
        
        # TASK C Integration Test: Fault Injection
        if self.inject_fault:
            error_msg = "PDF Engine Timeout: Unresponsive rendering service."
            log_trace(context.run_id, self.task_id, "error", "failure", {
        ...
        log_trace(context.run_id, self.task_id, "action_end", "success", {"bytes_rendered": 1024})
        return TaskResult(status="success", output_data=output)

class DeployTask(AgentTask):
    def execute(self, context: PipelineContext) -> TaskResult:
        log_trace(context.run_id, self.task_id, "action_start", "success", {"action": "upload_to_storage"})
        output = "https://storage.enterprise.local/docs/output.pdf"
        log_trace(context.run_id, self.task_id, "action_end", "success", {"url": output})
        return TaskResult(status="success", output_data=output)
    ...
    def run(self, initial_input: str) -> None:
        ...
        for index, task in enumerate(self.tasks):
            ...
            if result.status == "failure":
                ...
                print("Subsequent stages (e.g. ID-004-DEPLOY) have been blocked from executing to prevent cascading failures.")
                log_trace(run_id, "Orchestrator", "pipeline_halt", "error", {"failed_stage": task.task_id})
...
if __name__ == "__main__":
    ...
    pipeline_success.add_task(ConvertTask("ID-003-CONVERT", "Render PDF", inject_fault=False))
    ...
    pipeline_fail.add_task(ConvertTask("ID-003-CONVERT", "Render PDF", inject_fault=True)) # <--- FEHLER WIRD INJIZIERT
